    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
        <div className="border-l-[6px] border-[#E74C3C] pl-6 py-2">
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-black/50 mt-2">Full-stack Developer | Front-end Enthusiast</h2>
        </div>
        <div className="bg-black/5 p-4 border-2 border-black/10 rounded-sm text-sm font-bold w-full sm:w-auto">
          <p className="flex justify-between gap-8"><span>LOCATION:</span> <span>Banda Aceh, Indonesia</span></p>
          <p className="flex justify-between gap-8"><span>GUILD:</span> <span>Open to Work</span></p>
          <p className="flex justify-between gap-8"><span>XP:</span> <span>4+ Years</span></p>
        </div>
      </div>
      ...
      <div className="bg-white/50 p-6 border-2 border-black/5 rounded-sm">
         <p className="text-lg sm:text-xl font-medium leading-relaxed italic">
           "Passionate Software Developer with a strong enthusiasm for frontend development. Highly skilled in building cross-platform mobile applications with Flutter & Dart, and robust web applications using Laravel & PHP. Experienced in craft layouts using React.js, Bootstrap, and standard HTML/CSS/JS."
         </p>
      </div>

      <div className="flex flex-wrap gap-4 pt-4">
        <a href="mailto:yafiazka19@gmail.com" className="px-6 sm:px-8 py-3 sm:py-4 bg-[#E74C3C] border-4 border-black shadow-[4px_4px_0px_0px_#000] active:translate-y-1 active:shadow-none transition-all font-black uppercase text-white inline-flex items-center gap-2 group">
